Hooks.once('init', async () => {
    ...
    game.settings.register("dragupload", "canUpload", {
        name: "Can use Dragupload",
        config: true,
        type: Boolean,
        scope: 'world',
        default: true
...
Hooks.on("getSceneControlButtons", (controls) => {
    ...
    if (game.user.isGM) {
        const toggle = {
            ...
            title: 'Can Dragupload',
            icon: 'fas fa-wifi',
            toggle: true,
            active: game.settings.get("dragupload", "canUpload"),
            onClick: (toggle) => {

                let newState = !game.settings.get("dragupload", "canUpload")
                game.settings.set("dragupload", "canUpload", newState)
                if(newState == true){
                    bindDragDrop();
                }
            }
...
async function canUpload(){
    let canupload = game.settings.get('dragupload', 'canUpload')

    if(canupload){
        await bindDragDrop();
    }
    else{
        await unbindDragDrop();
    }
...
async function bindDragDrop(){
    window.dragUpload.DragDrop = new DragDrop({
        permissions: {
            drop: canUpload
        },
        callbacks: {
    ...
    window.dragUpload.DragDrop.bind(document.getElementById("board"));
}
async function unbindDragDrop(){
    window.dragUpload.DragDrop = {};
}
...
async function CreateActorWithType(event, data, tokenImageData, type) {
    ...
    if(type === "scene"){
        ...
        const newScene = await Scene.create({name: data.name, navName: "A New Scene", img: data.img, width: data.width, height: data.height, globalLight: true, 
                                            fogExploration: false, tokenVision: false, gridType: 0, padding: 0.00}); 

        newScene.sheet.render(true);
    ...
    const actualToken = canvas.scene.tokens.find(t => t.name === actorName);
    window.Tokenizer.tokenizeSceneToken({ token: actualToken, actor: actualToken.actor });
}
